...
import PKTCard from '../components/PKTCard'
...
function PackageTrip() {
  ...
  return (
    <Box>
      <AppBar></AppBar>
      <Box>
        <img src="PackageTrip.png" width={"100%"} />
      </Box>
      <Box
        sx={{
          '& > :not(style)':
      ...
      <div>
        ...
        >
          {filteredTrips.map((PKT, index) => (
            <Grid item xs={2} sm={4} md={4} lg={3} xl={2} key={index}>
              <PKTCard packageTrip={PKT} onUpdatepackageTrip={onUpdatePackagetrip} />
            </Grid>
          ))}
        </Grid>
      </div>
